...
/** The gateway always sends a code; showing it beats a generic apology the operator cannot act on. */
...
function correlationId(error: ApolloError): string | null {
  const extensions = error.graphQLErrors[0]?.extensions;
  const id = extensions?.['correlationId'];
  ...
  return (
    <>
      <div className="empty">
        ...
        <span className="t" style={{ color: 'var(--blush-d)' }}>
          {code}
        </span>
        <span className="s">
      ...
      <div className="msg err">
        <div className="h">&#9632; extensions.code = {code}</div>
        <div className="m">{error.message}</div>
        {correlation !== null && <div className="c">correlationId {correlation}</div>}
      </div>
    </>
  );
}
