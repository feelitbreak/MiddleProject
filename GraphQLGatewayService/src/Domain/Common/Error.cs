...
public sealed class Error
{
    ...
    /// <summary>Gets a value indicating whether retrying the identical operation could succeed.</summary>
    public bool IsRetryable => this.Code == ErrorCode.Transient;

    /// <summary>Creates a <see cref="ErrorCode.Transient"/> error.</summary>
    public static Error CreateTransient(string description) =>
        new(ErrorCode.Transient, description);

    /// <summary>Creates a <see cref="ErrorCode.Permanent"/> error.</summary>
    public static Error CreatePermanent(string description) =>
        new(ErrorCode.Permanent, description);
